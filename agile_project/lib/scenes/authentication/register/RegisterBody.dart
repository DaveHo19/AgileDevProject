...
class _RegisterBodyState extends State<RegisterBody> {
  ...
  Widget build(BuildContext context){
    return isLoading ? const Loading() : Background(
      child: Column(
    ...
    if (isFilledAll(email, psdOne, psdTwo)){
      if(validateEmail(email)){
        if(psdOne == psdTwo){
          ...
          dynamic result = await authService.registerWithEmailAndPassword(email, psdOne);
          if(result != null){
            isLoading = false;
            ...
            setState(() {
              isLoading = false;
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Failed to register with the email and password")));
            });
          }
        } else {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Both password are differences!")));
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Invalid format for email address!")));
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("There are some field is empty!")));
    }
  }
