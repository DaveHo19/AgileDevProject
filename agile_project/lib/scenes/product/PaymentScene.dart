...
class MyPaymentScene extends StatefulWidget {
  const MyPaymentScene({Key? key, required this.orderData}) : super(key: key);
...
class _MyPaymentSceneState extends State<MyPaymentScene> {
  ...
  void initState() {
    ...
    super.initState();
    orderBookIDList =
        widget.orderData.orderItems.entries.map((e) => e.key).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: (user != null)
          ? buildContent()
          : const Center(child: Text("No user login")),
    );
  }
...
//bank card info input
  Widget buildCardInfoRow() {
    return Container(
        ...
        child: Row(
          children: [
            Expanded(child: buildField("Expire Date", cardExpireDate)),
            Expanded(child: buildField("Card CVC", cardCVCController)),
          ],
        ));
  }
    ...
    if (formKey.currentState!.validate()) {
      if (user != null) {
        ...
        if (id.isNotEmpty) {
          ...
          curOrderList.add(id);
          dynamic result =
              await dbService.updateUserOrder(user!.uid, curOrderList);
          if (result == null) {
            bookList =
                await dbService.getBookListByBookISBNList(orderBookIDList);

            //update new quantity in the order
            for (int i = 0; i < bookList.length; i++) {
              int newQuantity = bookList[i].quantity -
                  widget.orderData.orderItems[bookList[i].ISBN_13]!;
              dynamic result = await dbService.updateBookQuantity(
                  bookList[i].ISBN_13, newQuantity);
              if (result != null) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text("Failed to change book quantity")));
              }
            }
